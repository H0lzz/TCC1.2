...
Além disso, foram coletados, em buscas na internet, quatro livros e um artigo para dar base às referências teóricas deste trabalho.
...
\begin{itemize}
    ...
    \item \textit{Usability Engineering}
    \item \textit{The Pomodoro Technique: The Life-Changing Time-Management System}
    \item \textit{Optimization of repetition spacing in the practice of learning}
\end{itemize}

\section{Gestão do Tempo}

A gestão do tempo é um conceito fundamental no ambiente de preparação para provas, sendo definida como a disposição e a capacidade do indivíduo de planejar, agendar e executar atividades de estudo de forma eficaz.
...
A GBL demonstrou ser uma metodologia eficaz para o desenvolvimento do pensamento computacional em estudantes do ensino fundamental \cite{hsu_2026}.
...
Para maximizar os benefícios da gamificação, é necessário que ela seja integrada a métodos de aprendizado eficazes.
A combinação do aprendizado baseado em jogos com métodos de orientação, como o 5W1H (o quê, quem, quando, onde, por que e como) ou o mapeamento de conceitos baseado em associação (CABCM), demonstrou ser eficaz na promoção do pensamento computacional e da autoeficácia de estudantes \cite{hsu_2026}.
...
Foi feito um \textit{benchmark} sobre aplicativos similares com objetivos de auxiliar na rotina de estudos dos estudantes.
...
Ao focar em métricas e estatísticas, o Aprovado se alinha com o pilar de gestão do tempo deste trabalho, fornecendo uma ideia de onde o tempo está sendo aplicado, o que é relevante para a organização da rotina de um estudante \cite{aprovado_2026}. Porém, o aplicativo não se concentra no campo da saúde mental do estudante e, logo, não traz uma visão da qualidade do estudo.
...
Além disso, o aplicativo Anki, mostrado na Figura~\ref{fig:anki}, é referência em repetição espaçada, oferecendo um sistema de \textit{flashcards} com algoritmo que agenda a revisão de cada cartão para quando o usuário está mais propenso a esquecer a informação{} \cite{anki_2026}.
Apesar de ser o padrão de mercado nesse pilar específico, o Anki não possui algum recurso nativo de gestão de tempo, de gamificação ou de monitoramento de bem-estar, concentrando-se exclusivamente na otimização da memorização \cite{anki_2026}.

\begin{figure}[H]
    \centering
    \caption{Tela inicial do Anki (AnkiDroid)}
    \includegraphics[width=0.75\linewidth]{Imagens/Anki.jpg}
    \par
    Fonte: \cite{anki_2026}.
    \par
    \label{fig:anki}
\end{figure}

Outro exemplo é o aplicativo Habitica, que adota a abordagem de ser uma plataforma de gamificação de hábitos e tarefas no formato de um jogo, na qual o usuário ganha experiência, ouro e itens ao concluir tarefas do cotidiano, além de poder participar de grupos e missões em equipe \cite{habitica_2026}.
Por ser uma ferramenta de produtividade genérica, e não voltada especificamente ao estudo, o Habitica não possui repetição espaçada, \textit{flashcards}, cronômetro Pomodoro ou \textit{check-ins} de humor. Sua gamificação se aplica a qualquer tipo de tarefa que o usuário cadastre, sem qualquer relação com técnicas de aprendizagem \cite{habitica_2026}.

\begin{figure}[H]
    \centering
    \caption{Tela inicial do Habitica}
    \includegraphics[width=0.75\linewidth]{Imagens/Habitica.jpg}
    \par
    Fonte: \cite{habitica_2026}.
    \par
    \label{fig:habitica}
\end{figure}

Por fim, o aplicativo Koki \textit{FlashCards}, mostrado na Figura~\ref{fig:koki}, é um aplicativo que aplica um algoritmo de repetição espaçada similar ao Anki, porém voltado à memorização de vocabulário de idiomas, com baralhos prontos para exames \cite{koki_2026}.
Diferentemente do Anki, o Koki incorpora elementos leves de gamificação, como sequência de dias de uso (\textit{streak}) e pontuação, além de um plano de assinatura \textit{premium} \cite{koki_2026}.
Ainda assim, por ser voltado exclusivamente à aprendizagem de vocabulário de idiomas, o Koki não possui cronômetro Pomodoro, gestão geral de tempo de estudo ou \textit{check-ins} de bem-estar, e seus baralhos prontos não se aplicam ao conteúdo específico de disciplinas universitárias.

\begin{figure}[H]
    \centering
    \caption{Tela inicial do Koki \textit{FlashCards}}
    \includegraphics[width=0.75\linewidth]{Imagens/KokiFlashcards.jpg}
    \par
    Fonte: \cite{koki_2026}.
    \par
    \label{fig:koki}
\end{figure}

Portanto, o aplicativo proposto para este trabalho se diferencia dos concorrentes ao buscar uma integração completa de todos os valores mencionados anteriormente, em vez de se aprofundar isoladamente em um ou dois pilares.
Enquanto o Aprovado se restringe ao registro e à estatística da gestão do tempo, o StudyTok se destaca na geração de \textit{flashcards} por IA e gamificação, o Anki é o padrão de repetição espaçada, o Habitica é o padrão de gamificação de hábitos e o Koki combina repetição espaçada com gamificação leve voltada exclusivamente a vocabulário de idiomas, nenhum deles combina todos os propósitos com o controle de bem-estar do estudante.
O aplicativo proposto visa unir esses valores a fim de trazer um produto centralizado e desfragmentado, evidenciando de forma mais robusta a lacuna de mercado que motiva este trabalho.
O diferencial está em combinar a eficácia da repetição espaçada com uma ferramenta de cronômetro Pomodoro, elementos de gamificação e \textit{check-ins} de humor, oferecendo uma solução mais adaptada às demandas do estudante universitário.

A Tabela~\ref{tab:benchmark} resume comparativamente os propósitos deste trabalho presentes em cada um dos cinco concorrentes analisados, evidenciando visualmente a lacuna de mercado descrita ao longo desta seção.

\begin{table}[H]
\centering
\caption{Comparativo entre o aplicativo proposto e os concorrentes analisados}
\label{tab:benchmark}
\begin{tabularx}{\textwidth}{|X|>{\centering\arraybackslash}p{2.1cm}|>{\centering\arraybackslash}p{2.1cm}|>{\centering\arraybackslash}p{2.4cm}|>{\centering\arraybackslash}p{2.1cm}|>{\centering\arraybackslash}p{2.3cm}|}
\hline
\textbf{Aplicativo} & \textbf{Gestão do tempo} & \textbf{SRS / \textit{Flashcards}} & \textbf{Gamificação} & \textbf{Bem-estar} & \textbf{Conteúdo genérico universitário} \\
\hline
Aprovado & Sim & Não & Não & Não & Sim \\
\hline
StudyTok & Não & Sim & Sim & Não & Sim \\
\hline
Anki & Não & Sim & Não & Não & Sim \\
\hline
Habitica & Não & Não & Sim & Não & Sim \\
\hline
Koki \textit{FlashCards} & Não & Sim & Sim & Não & Não \\
\hline
\textbf{Focus (proposto)} & Sim & Sim & Sim & Sim & Sim \\
\hline
\multicolumn{6}{r}{\small Fonte: autoria própria.} \\
\end{tabularx}
\end{table}
...
A boa usabilidade de um sistema é construída a partir de um conjunto de regras reconhecidas no campo da engenharia de \textit{software} como as heurísticas de Nielsen \cite{nielsen_1994}.
...
Dessa forma, este trabalho se compromete a aplicar essas heurísticas como guia de \textit{design}, buscando favorecer a usabilidade e a acessibilidade para diferentes públicos.
Além disso, o projeto observa os princípios da Lei Geral de Proteção de Dados (LGPD) \cite{brasil2018lgpd}, orientando as decisões de tratamento das informações dos usuários; a Seção~\ref{sec:seguranca_privacidade} detalha as medidas técnicas planejadas para essa conformidade.
...
Diante disso, o ARE calcula um intervalo de revisão para o usuário conforme a dificuldade percebida, buscando apresentar o conteúdo próximo ao momento em que estaria sendo esquecido, o que a literatura aponta como mais eficaz do que a repetição excessiva.
A implementação dessa metodologia ganhou popularidade em aplicações móveis, pois a portabilidade dos \textit{smartphones} reduz as limitações de tempo e local, permitindo que estudantes estudem em momentos e locais mais flexíveis.
...
Esse princípio deriva dos estudos de \citeonline{wozniak_1994} sobre o espaçamento ótimo de repetições na prática do aprendizado, que fundamentam o algoritmo SuperMemo.
Para este trabalho pretende-se utilizar uma variação do algoritmo dinâmico SuperMemo 2 (SM-2), em que os dois primeiros intervalos são fixos e os seguintes são calculados multiplicando o intervalo anterior pelo fator de facilidade (calculado com base na nota de facilidade fornecida pelo usuário).
...
Criada por Francesco Cirillo \cite{cirillo_2018}, a técnica divide o tempo em intervalos de 25 minutos de foco, seguidos por 5 minutos de descanso.
...
\section{Autossabotagem e Adesão ao Uso do Aplicativo}
\label{sec:autossabotagem}

Além de organizar a rotina de estudos, um aplicativo desse tipo precisa lidar com um problema comportamental documentado na literatura: o abandono do próprio aplicativo antes que seus benefícios se consolidem.
Uma revisão de escopo recente, que analisou dezoito estudos e mais de quinhentos mil participantes, identificou que aproximadamente 70\% dos usuários interrompem o uso de aplicativos de comportamento e saúde mental dentro dos primeiros 100 dias, com desistências mais acentuadas logo nos estágios iniciais \cite{kidman_2024}.
Esse padrão de abandono precoce está diretamente ligado ao objetivo de negócio deste trabalho de minimizar a taxa de abandono, e motiva a incorporação de mecanismos de \textit{design} que combatam formas específicas de autossabotagem do usuário, e não apenas elementos de gamificação.

O primeiro desses mecanismos combate o chamado \textit{what the hell effect} (efeito "que se dane"), descrito originalmente por \citeonline{polivy_1985} no contexto de dietas: ao perceber uma pequena falha (por exemplo, pular um dia de dieta), o indivíduo tende a interpretá-la como uma quebra total do compromisso e abandona o esforço por completo, em vez de simplesmente retomá-lo no dia seguinte.
O mesmo padrão se manifesta em sistemas de sequências (\textit{streaks}) de hábitos e estudo: ao perder um único dia, o usuário frequentemente vê sua sequência zerada como prova de fracasso e desiste de continuar usando o aplicativo.
Para mitigar esse efeito, o aplicativo planeja implementar um mecanismo de congelamento de \textit{streak}, que perdoa a perda de um único dia por semana sem zerar a sequência do usuário, permitindo que uma falha pontual seja tratada como uma exceção esperada, e não como o fim do esforço.

O segundo mecanismo busca reduzir a sobrecarga de decisão no momento em que o usuário retoma os estudos após um período de ausência.
A literatura sobre sobrecarga de escolha (\textit{choice overload}) demonstra que, quando confrontado com um número excessivo de opções ou itens pendentes, o indivíduo tende a adiar ou evitar a decisão de agir, em vez de se sentir motivado por ela \cite{chernev_2015}.
Aplicado ao contexto deste trabalho, um usuário que se ausenta por alguns dias e retorna ao aplicativo para encontrar dezenas de revisões de \textit{flashcards} acumuladas tende a se sentir sobrecarregado e a adiar ou evitar a sessão de estudo.
Por isso, em vez de exibir de uma só vez todas as revisões pendentes, o aplicativo planeja limitar a lista exibida por sessão a um lote reduzido e gerenciável, revelando o restante apenas gradualmente, à medida que o usuário avança.

Por fim, o terceiro mecanismo estende a funcionalidade de \textit{check-in} de humor descrita na seção anterior de um simples registro passivo para uma intervenção ativa.
Quando o sistema identifica um padrão recorrente de \textit{check-ins} negativos (níveis de estresse elevados ou humor baixo ao longo de vários registros consecutivos), o aplicativo planeja emitir um alerta com uma sugestão concreta e de baixo esforço, como encurtar a próxima sessão de foco ou fazer uma pausa, em vez de deixar que o usuário continue se cobrando no mesmo ritmo.
Essa resposta é coerente com a própria lógica da SRL descrita anteriormente: ao tornar visível o próprio estado emocional, o usuário ganha a oportunidade de ajustar seu comportamento antes que o desgaste o leve a abandonar os estudos por completo \cite{liu_2025}.
